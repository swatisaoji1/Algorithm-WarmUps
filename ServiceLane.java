...
public class ServiceLane {
	...
	public static void main(String[] args) {
		...
		int hwLength = sc.nextInt();
		int testCases = sc.nextInt();
		int[] width = readArray(sc, hwLength);
		for(int i = 0; i < testCases; i++ ){
			int entry = sc.nextInt();
			int exit = sc.nextInt();
			System.out.println(largestVehicle(width, entry, exit));
		}
	}

	private static int largestVehicle(int[] width, int entry, int exit) {
		int min = 10;
	...
	private static int[] readArray(Scanner sc, int hwLength) {
		int[] width = new int[hwLength];
